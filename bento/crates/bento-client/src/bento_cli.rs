...
async fn main() -> Result<()> {
    ...
    let client =
        ProvingClient::from_parts(args.endpoint.clone(), api_key.clone(), risc0_zkvm::VERSION)
            .unwrap();
...
async fn sp1_workflow(
    ...
    let create_resp: Sp1CreateResp = create_req
        .json(&Sp1CreateReq { image: image_id_str.clone(), input: input_id_str.clone() })
        .send()
        .await
