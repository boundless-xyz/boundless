...
use risc0_zkvm::{
    GenericReceipt, ReceiptClaim, SegmentReceipt, SuccinctReceipt, Unknown, WorkClaim,
};
use uuid::Uuid;
...
pub async fn resolve_povw(
    ...
    // Try to deserialize as segment receipt first, then lift it to POVW if needed
    let povw_receipt: SuccinctReceipt<WorkClaim<ReceiptClaim>> =
        match deserialize_obj::<SegmentReceipt>(&receipt) {
            Ok(segment_receipt) => {
                // Successfully deserialized as segment receipt, now lift it to POVW
                agent
                    .prover
                    .as_ref()
                    .context("Missing prover from POVW resolve task")?
                    .lift_povw(&segment_receipt)
                    .context("Failed to lift segment receipt to POVW")?
            }
            Err(_) => {
                // Failed to deserialize as segment receipt, try as already-lifted POVW receipt
        ...
        && !guest_output.assumptions.is_empty()
    {
        ...
        for assumption in assumptions {
            ...
            let assumption_key = format!("{job_prefix}:{RECUR_RECEIPT_PATH}:{assumption_claim}");
            ...
            let assumption_receipt = deserialize_obj(&assumption_bytes).with_context(|| {
                format!(
