...
impl Agent {
    ...
    pub async fn poll_work(&self) -> Result<()> {
        ...
        if self.args.monitor_requeue {
            ...
            tokio::spawn(async move {
                Self::poll_for_requeue(term_sig_copy).await.expect("Requeue failed")
            });
        }

        while !term_sig.load(Ordering::Relaxed) {
            let task = tdb::request_work(&self.args.task_stream)
                .await
            ...
            if let Err(err) = self.process_work(&task).await {
                ...
                if task.max_retries > 0 {
                    ...
                    // For Redis-only implementation, we'll use a simpler retry logic
                    if !tdb::update_task_retry(&task.job_id, &task.task_id)
                        .await
                    ...
                    tdb::update_task_failed(&task.job_id, &task.task_id, &err_str)
                        .await
    ...
    pub async fn process_work(&self, task: &ReadyTask) -> Result<()> {
        ...
        tdb::update_task_done(&task.job_id, &task.task_id, res)
            .await
    ...
    async fn poll_for_requeue(term_sig: Arc<AtomicBool>) -> Result<()> {
        while !term_sig.load(Ordering::Relaxed) {
            tracing::debug!("Triggering a requeue job...");
            let retry_tasks = tdb::requeue_tasks(100).await?;
            if retry_tasks > 0 {
