-- -- Update request_work to return from the first non-empty stream
-- -- Prioritizes worker types and streams to find an available task
...
-- BEGIN
--   -- Iterate through worker types in order
--   FOREACH curr_worker_type IN ARRAY in_worker_types LOOP
--     -- Find the highest priority stream for this worker type with ready tasks
--     SELECT streams.id INTO stream
--     FROM streams
--     JOIN tasks ON tasks.stream_id = streams.id
--     WHERE streams.worker_type = curr_worker_type
--       AND tasks.state = 'ready'
--     ORDER BY streams.priority
--     LIMIT 1
--     FOR UPDATE SKIP LOCKED;

--     -- If a stream with ready tasks is found, attempt to select a task
--     IF stream IS NOT NULL THEN
--       -- Atomically select and update the task
...
--         update_task.job_id, 
--         update_task.task_id, 
--         update_task.task_def, 
--         update_task.max_retries
--       FROM update_task;

--       -- If a task was found, return it and exit the function
...
--         prereqs := '[]';
--         RETURN NEXT;
--         RETURN;  -- Immediately exit after finding a task
--       END IF;
--     END IF;
--   END LOOP;

--   -- If no tasks are found across all worker types, nothing is returned
-- END;
-- $$ LANGUAGE plpgsql;
