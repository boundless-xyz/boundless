-- Migration 7: Add NOTIFY support, batch work request, and failed job cleanup
--
-- This migration adds:
-- 1. NOTIFY in create_task() when tasks become ready
-- 2. request_work_batch() function for pulling multiple tasks at once
-- 3. fail_tasks_from_failed_jobs() function for periodic cleanup

-- Drop and recreate create_task with NOTIFY support
...
BEGIN
  ...
  -- If task is ready, send notification to wake up workers
  IF not_done_count = 0 THEN
    SELECT worker_type INTO worker_type_var FROM streams WHERE id = stream_id;
    PERFORM pg_notify('task_ready', worker_type_var);
  END IF;
END;
$$ LANGUAGE plpgsql;

-- Update update_task_done to send NOTIFY when child tasks become ready
...
DECLARE
  ...
  unresolved INTEGER;
  ready_stream_id UUID;
  ready_worker_type TEXT;
BEGIN
  UPDATE tasks SET
    state = 'done',
    output = output_var,
  ...
  IF found_done_task THEN
    ...
    -- Also notify workers for streams with newly ready tasks
    FOR ready_stream_id, ready_worker_type IN
      WITH child_tasks AS (
        SELECT tasks.job_id, tasks.task_id, tasks.waiting_on, tasks.stream_id
          FROM tasks
          JOIN task_deps
      ...
      updated_tasks AS (
        ...
        RETURNING tasks.stream_id, (child_tasks.waiting_on - 1) as new_waiting_on
      )
      SELECT DISTINCT s.id, s.worker_type
      FROM updated_tasks ut
      JOIN streams s ON s.id = ut.stream_id
      WHERE ut.new_waiting_on = 0
    LOOP
      PERFORM pg_notify('task_ready', ready_worker_type);
    END LOOP;
