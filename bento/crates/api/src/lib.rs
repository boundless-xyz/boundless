...
use std::sync::Arc;
use taskdb_redis::{JobState, RedisTaskDB, TaskDbErr};
use thiserror::Error;
use uuid::Uuid;
use workflow_common::{
...
async fn prove_stark(
    ...
    let job_id = {
        let mut taskdb = RedisTaskDB::new(&state.redis_url)
            .await
        ...
        let (_aux_stream, exec_stream, _gpu_prove_stream, _gpu_coproc_stream, _gpu_join_stream) =
            helpers::get_or_create_streams(&mut taskdb, &api_key)
                .await
                .context("Failed to get / create steams")?;

        taskdb
            .create_job(&exec_stream, &task_def, state.exec_retries, state.exec_timeout, &api_key)
            .await
...
async fn stark_status(
    ...
    let (job_state, exec_stats, error_msg) = {
        let mut taskdb = RedisTaskDB::new(&state.redis_url)
            .await
            .context("Failed to create TaskDB connection")?;
        let job_state =
            taskdb.get_job_state(&job_id, &api_key).await.context("Failed to get job state")?;

        let exec_stats = if job_state == JobState::Done {
            Some(
                helpers::get_exec_stats(&mut taskdb, &job_id)
                    .await
        ...
        let error_msg = if job_state == JobState::Failed {
            Some(taskdb.get_job_failure(&job_id).await.context("Failed to get job error message")?)
        } else {
            None
        };

        (job_state, exec_stats, error_msg)
    };

    let (exec_stats, receipt_url) = if job_state == JobState::Done {
        let exec_stats = exec_stats.unwrap();
        (
            Some(SessionStats {
...
async fn groth16_status(
    ...
    let (job_state, error_msg, output) = {
        let mut taskdb = RedisTaskDB::new(&state.redis_url)
            .await
            .context("Failed to create TaskDB connection")?;
        let job_state =
