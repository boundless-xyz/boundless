...
use workflow_common::{
    ...
    s3::{
        ...
        RECEIPT_BUCKET_DIR, S3Client, SHRINK_BLAKE3_BUCKET_DIR, STARK_BUCKET_DIR,
...
const SHRINK_BLAKE3_START_PATH: &str = "/shrink_blake3/create";
async fn shrink_blake3(
    State(state): State<Arc<AppState>>,
    ExtractApiKey(api_key): ExtractApiKey,
    Json(start_req): Json<SnarkReq>,
) -> Result<Json<CreateSessRes>, AppError> {
    ...
    let task_def = serde_json::to_value(TaskType::Snark(WorkflowSnarkReq {
        receipt: start_req.session_id,
        compress_type: CompressType::ShrinkBlake3,
    }))
    .context("Failed to serialize ExecutorReq")?;
...
const SHRINK_BLAKE3_STATUS_PATH: &str = "/shrink_blake3/status/:job_id";
async fn shrink_blake3_status(
    State(state): State<Arc<AppState>>,
    ExtractApiKey(api_key): ExtractApiKey,
    Path(job_id): Path<Uuid>,
    Host(hostname): Host,
    ...
    let (error_msg, output) = match job_state {
        ...
        JobState::Done => {
            (None, Some(format!("http://{hostname}/receipts/shrink_blake3/receipt/{job_id}")))
        }
        JobState::Failed => (
            Some(
                taskdb::get_job_failure(&state.db_pool, &job_id)
...
const GET_SHRINK_BLAKE3_PATH: &str = "/receipts/shrink_blake3/receipt/:job_id";
async fn shrink_blake3_download(
    State(state): State<Arc<AppState>>,
    Path(job_id): Path<Uuid>,
) -> Result<Vec<u8>, AppError> {
    let receipt_key = format!("{RECEIPT_BUCKET_DIR}/{SHRINK_BLAKE3_BUCKET_DIR}/{job_id}.bincode");
    if !state
        .s3_client
        .object_exists(&receipt_key)
        .await
...
pub fn app(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(SHRINK_BLAKE3_START_PATH, post(shrink_blake3))
        .route(SHRINK_BLAKE3_STATUS_PATH, get(shrink_blake3_status))
        .route(GET_GROTH16_PATH, get(groth16_download))
        .route(GET_SHRINK_BLAKE3_PATH, get(shrink_blake3_download))
