...
const BRANCH_NAME = "launch-infra";
...
export class LOrderGeneratorPipeline extends pulumi.ComponentResource {
  constructor(name: string, args: LOrderGeneratorPipelineArgs, opts?: pulumi.ComponentResourceOptions) {
    super(`boundless:pipelines:l-${APP_NAME}Pipeline`, name, args, opts);
    ...
    const githubTokenSecret = new aws.secretsmanager.Secret(`l-${APP_NAME}-ghToken`);
    const dockerTokenSecret = new aws.secretsmanager.Secret(`l-${APP_NAME}-dockerToken`);

    new aws.secretsmanager.SecretVersion(`l-${APP_NAME}-ghTokenVersion`, {
      secretId: githubTokenSecret.id,
      secretString: githubToken,
    });

    new aws.secretsmanager.SecretVersion(`l-${APP_NAME}-dockerTokenVersion`, {
      secretId: dockerTokenSecret.id,
      secretString: dockerToken,
    });

    new aws.iam.RolePolicy(`l-${APP_NAME}-build-secrets`, {
      role: role.id,
      policy: {
        Version: '2012-10-17',
        Statement: [
    ...
    const stagingDeploymentBaseSepolia = new aws.codebuild.Project(
      `l-${APP_NAME}-staging-84532-build`,
      this.codeBuildProjectArgs(APP_NAME, "l-staging-84532", role, BOUNDLESS_STAGING_DEPLOYMENT_ROLE_ARN, dockerUsername, dockerTokenSecret, githubTokenSecret),
      { dependsOn: [role] }
    );

    const prodDeploymentBaseSepolia = new aws.codebuild.Project(
      `l-${APP_NAME}-prod-84532-build`,
      this.codeBuildProjectArgs(APP_NAME, "l-prod-84532", role, BOUNDLESS_PROD_DEPLOYMENT_ROLE_ARN, dockerUsername, dockerTokenSecret, githubTokenSecret),
      { dependsOn: [role] }
    );

    const prodDeploymentBaseMainnet = new aws.codebuild.Project(
      `l-${APP_NAME}-prod-8453-build`,
      this.codeBuildProjectArgs(APP_NAME, "l-prod-8453", role, BOUNDLESS_PROD_DEPLOYMENT_ROLE_ARN, dockerUsername, dockerTokenSecret, githubTokenSecret),
      { dependsOn: [role] }
    );

    const prodDeploymentEthSepolia = new aws.codebuild.Project(
      `l-${APP_NAME}-prod-11155111-build`,
      this.codeBuildProjectArgs(APP_NAME, "l-prod-11155111", role, BOUNDLESS_PROD_DEPLOYMENT_ROLE_ARN, dockerUsername, dockerTokenSecret, githubTokenSecret),
      { dependsOn: [role] }
    );

    const pipeline = new aws.codepipeline.Pipeline(`l-${APP_NAME}-pipeline`, {
      pipelineType: "V2",
      artifactStores: [{
        type: "S3",
    ...
    new aws.codestarnotifications.NotificationRule(`l-${APP_NAME}-pipeline-notifications`, {
  ...
  private codeBuildProjectArgs(
    ...
    return {
      buildTimeout: 60,
      description: `Launch deployment for ${APP_NAME}`,
