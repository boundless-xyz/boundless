-- 002_dedupe_on_order_id.sql
-- Switch deduplication keys from request_digest to order_id.
-- A single request_digest can be evaluated twice (once per fulfillment type),
-- so order_id (which encodes request_id + digest + fulfillment_type) is the
-- correct natural key.
-- Also adds new columns to match the current telemetry structs.
-- Redshift does not allow CREATE OR REPLACE VIEW when the column count changes,
...
SELECT
    ...
    fulfillment_type, preflight_cache_hit, queue_duration_ms,
    preflight_duration_ms, received_at_timestamp, evaluated_at, received_at
FROM (
    SELECT
        ...
        data.estimated_total_proving_time_secs::BIGINT AS estimated_proving_time_secs,
...
SELECT
    ...
    lock_duration_secs, proving_duration_secs, aggregation_duration_secs,
    submission_duration_secs, total_duration_secs,
    estimated_proving_time_secs, actual_proving_time_secs,
...
FROM (
    SELECT
        ...
        data.estimated_total_proving_time_secs::BIGINT      AS estimated_proving_time_secs,
        data.actual_total_proving_time_secs::BIGINT         AS actual_proving_time_secs,
...
INSERT INTO telemetry._migrations (version, name) VALUES (2, '002_dedupe_on_order_id');
