...
use boundless_signer::{
    GenericSigner, HttpRemoteSignerBackend, LocalSignerBackend, SignerBackend, SignerError,
    SignerRole,
};
...
fn make_local_backend(wallet: PrivateKeySigner) -> Arc<GenericSigner> {
    let provider =
...
async fn make_remote_backend(base_url: &str) -> Arc<GenericSigner> {
    Arc::new(GenericSigner::Remote(
...
async fn http_sign_hash_parity() {
    ...
    let sig_local = SignerBackend::sign_hash(local.as_ref(), hash).await.unwrap();
    let sig_remote = SignerBackend::sign_hash(remote.as_ref(), hash).await.unwrap();
...
async fn http_sign_message_parity() {
    ...
    let sig_local = SignerBackend::sign_message(local.as_ref(), msg).await.unwrap();
    let sig_remote = SignerBackend::sign_message(remote.as_ref(), msg).await.unwrap();

    assert_eq!(sig_local, sig_remote, "sign_message: HTTP remote must match local backend");
}

// ── Signer trait parity (sign_hash via GenericSigner::Signer impl) ────────────

#[tokio::test]
async fn http_sign_via_signer_trait_parity() {
    ...
    // GenericSigner implements alloy::signers::Signer directly.
    let sig_local = Signer::sign_hash(local.as_ref(), &hash).await.unwrap();
    let sig_remote = Signer::sign_hash(remote.as_ref(), &hash).await.unwrap();

    assert_eq!(
        sig_local, sig_remote,
        "sign_hash via Signer trait: HTTP remote must match local GenericSigner"
    );
}

// ── error mapping ─────────────────────────────────────────────────────────────

/// Creates a `GenericSigner::Remote` pointed at a mock server that has been
...
async fn make_error_backend(capabilities_addr: &str, post_status: u16) -> (MockServer, Arc<GenericSigner>) {
    ...
    let backend = Arc::new(GenericSigner::Remote(
        HttpRemoteSignerBackend::new_with_timeout(
            &server.base_url(),
            SignerRole::Prover,
            TEST_TIMEOUT,
        )
...
async fn http_error_403_maps_to_rejected() {
    ...
    let err = SignerBackend::sign_hash(backend.as_ref(), B256::ZERO).await.unwrap_err();
    assert!(
