...
//! Parity tests: `LocalSignerBackend` wrapped in `GenericSigner` must produce
//! byte-identical signatures to `PrivateKeySigner` for every signing surface
//! used in production.  `GenericSigner` implements `alloy::signers::Signer` and
//! `TxSigner<Signature>` directly, so no bridge adapter is needed.
//!
...
use boundless_signer::{GenericSigner, LocalSignerBackend, SignerBackend};

/// Anvil account 0 — deterministic, well-known.
const TEST_KEY: &str = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";

/// Build a `GenericSigner::Local` using an offline HTTP provider.
/// Signing methods never touch the provider; the URL is never contacted.
fn make_backend(wallet: PrivateKeySigner) -> Arc<GenericSigner> {
    let provider =
        ProviderBuilder::new().connect_http("http://127.0.0.1:8545".parse().expect("valid url"));
    Arc::new(GenericSigner::Local(LocalSignerBackend::from_signer(wallet, provider)))
}
...
async fn sign_hash_parity() {
    ...
    let sig_backend = SignerBackend::sign_hash(backend.as_ref(), hash).await.unwrap();

    assert_eq!(
        sig_direct, sig_backend,
        "sign_hash: local backend must be byte-identical to direct PrivateKeySigner"
    );
}

#[tokio::test]
async fn sign_hash_via_signer_trait_parity() {
    let hash = B256::from([0xABu8; 32]);
    let wallet: PrivateKeySigner = TEST_KEY.parse().unwrap();

    let sig_direct = wallet.sign_hash(&hash).await.unwrap();
    // GenericSigner implements alloy::signers::Signer directly — no bridge needed.
    let backend = make_backend(wallet);
    let sig_generic = Signer::sign_hash(backend.as_ref(), &hash).await.unwrap();

    assert_eq!(
        sig_direct, sig_generic,
        "sign_hash via GenericSigner::Signer impl must equal direct PrivateKeySigner"
    );
}
...
async fn sign_message_parity() {
    ...
    let backend = make_backend(wallet);
    let sig_backend = SignerBackend::sign_message(backend.as_ref(), msg).await.unwrap();

    assert_eq!(sig_direct, sig_backend, "sign_message: local backend must be byte-identical");
}

#[tokio::test]
async fn sign_message_via_signer_trait_parity() {
    let msg = b"EIP-191 test message";
    let wallet: PrivateKeySigner = TEST_KEY.parse().unwrap();

    let sig_direct = wallet.sign_message(msg).await.unwrap();
    let backend = make_backend(wallet);
    let sig_generic = Signer::sign_message(backend.as_ref(), msg.as_slice()).await.unwrap();

    assert_eq!(
        sig_direct, sig_generic,
        "sign_message via GenericSigner::Signer impl must equal direct PrivateKeySigner"
    );
}
...
async fn tx_sign_eip1559_parity() {
    ...
    let mut tx_generic = eip1559_tx();

    let sig_direct = wallet.sign_transaction(&mut tx_direct).await.unwrap();
    let backend = make_backend(wallet);
    let sig_generic = TxSigner::sign_transaction(backend.as_ref(), &mut tx_generic).await.unwrap();

    assert_eq!(
        sig_direct, sig_generic,
        "EIP-1559 TxSigner::sign_transaction via GenericSigner must equal direct PrivateKeySigner"
    );
}
...
async fn tx_sign_legacy_parity() {
    ...
    assert_eq!(
        sig_direct, sig_generic,
        "Legacy (EIP-155) TxSigner::sign_transaction via GenericSigner must equal direct PrivateKeySigner"
    );
}
...
async fn eip712_hash_parity() {
    ...
    let backend = make_backend(wallet);
    let sig_generic = Signer::sign_hash(backend.as_ref(), &eip712_hash).await.unwrap();

    assert_eq!(
        sig_direct, sig_generic,
        "EIP-712 hash signing via GenericSigner must equal direct PrivateKeySigner"
    );
}
...
async fn sender_address_consistent() {
    ...
    assert_eq!(SignerBackend::sender_address(backend.as_ref()), expected_addr);
    assert_eq!(Signer::address(backend.as_ref()), expected_addr);
}
