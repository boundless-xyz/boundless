...
impl ProverGenerateConfig {
    ...
    async fn run_wizard(
        ...
        let priority_requestor_lists = if peak_prove_khz > 4000.0 {
            display.note(&format!("Based on your cluster's peak performance of {:.2} kHz, we recommend enabling the standard and large requestor lists", peak_prove_khz));
            vec![
            ...
            display.note(&format!("Based on your cluster's peak performance of {:.2} kHz, we recommend enabling the standard requestor list", peak_prove_khz));
            vec![
                "https://requestors.boundless.network/boundless-recommended-priority-list.standard.json".to_string(),
            ]
        };
    ...
    async fn query_market_pricing(
        ...
        display.note(&format!(
            "Querying blocks {} to {} in chunks of {}",
    ...
    fn strip_tagged_section(content: &str, opening_tag: &str, closing_tag: &str) -> String {
        if let Some(start) = content.find(opening_tag) {
            if let Some(end) = content[start..].find(closing_tag) {
                let section_end = start + end + closing_tag.len();
