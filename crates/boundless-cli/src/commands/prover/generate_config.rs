...
impl ProverGenerateConfig {
    ...
    fn generate_chain_override_toml(&self, chain: &ChainConfig) -> Result<()> {
        ...
        // Disable Chainlink on-chain oracle for chains without a feed.
        // Supported: Ethereum Mainnet (1), Sepolia (11155111), Base (8453), Base Sepolia (84532).
        let chainlink_supported_chains = [1u64, 11155111, 8453, 84532];
        if !chainlink_supported_chains.contains(&chain.chain_id) {
            content.push_str(
                "\n# Chainlink price feeds not available on this chain.\n\
                 [price_oracle]\n\
                 eth_usd = \"auto\"\n\
                 zkc_usd = \"auto\"\n\
                 \n\
