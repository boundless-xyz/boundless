...
impl OrderFulfiller {
    ...
    pub async fn fulfill(
        ...
        let orders_jobs = orders.iter().cloned().map(move |(req, sig)| {
            let prover = prover.clone();
            let supported_selectors = supported_selectors.clone();
            async move {
        ...
        for (i, result) in results.into_iter().enumerate() {
            ...
            let (proof_id, receipt, claim, claim_digest, fill) = result?;
