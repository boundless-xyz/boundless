...
//
// Originally taken from risc0-r0vm
...
pub fn rpc_system<StreamT: RpcStream>(
    stream: StreamT,
) -> (RpcSender<StreamT::WriteHalf>, RpcReceiver<StreamT::ReadHalf>) {
...
impl<StreamT: AsyncWrite + Unpin> RpcSender<StreamT> {
        ...
        FutT: Future<Output = ()> + Send + 'static,
    {
        ...
        self.send(msg, message_id, RpcMessageKind::ExpectsResponse).await
    }
    ...
    pub async fn tell(&self, msg: &impl Serialize) -> Result<()> {
        ...
        self.send(msg, message_id, RpcMessageKind::ExpectsNoResponse).await
    }
...
impl RpcRegistry {
    fn new() -> Self {
        Self { pending_requests: Default::default(), next_message_id: RpcMessageId::INITIAL }
    }
...
impl<StreamT> Clone for RpcSender<StreamT> {
    fn clone(&self) -> Self {
        Self { stream: self.stream.clone(), registry: self.registry.clone() }
    }
}

impl<StreamT: AsyncWrite + Unpin> RpcSender<StreamT> {
    fn new(stream: StreamT, registry: Arc<Mutex<RpcRegistry>>) -> Self {
        Self { stream: Arc::new(TokioMutex::new(stream)), registry }
    }
    ...
    async fn send(
        ...
        let header = RpcHeader { body_length: body_length as u32, message_id, kind };
        bincode::serialize_into(&mut buffer[0..RPC_HEADER_SIZE], &header).unwrap();

        // Send the message
        self.stream.lock().await.write_all(&buffer).await.context("error sending RPC message")?;
...
impl<StreamT: AsyncRead + Unpin> RpcReceiver<StreamT> {
    ...
    async fn receive_one<RemoteMsgT: DeserializeOwned, FutT: Future<Output = ()>>(
        ...
        self.stream.read_exact(&mut body).await.context("error reading RPC body")?;

        match header.kind {
            RpcMessageKind::ExpectsResponse => {
                let msg: RemoteMsgT = bincode::deserialize(&body).with_context(|| {
                    format!("error deserializing {}", std::any::type_name::<RemoteMsgT>())
                })?;
            ...
            RpcMessageKind::IsResponse => {
                let callback = self.registry.lock().unwrap().remove_request(header.message_id)?;
                callback(&body).await?;
            }
        }
...
mod tests {
    ...
    impl Fixture {
        async fn new() -> Self {
            ...
            Self { sender_a, sender_b, receiver_a, receiver_b }
        }
        ...
        async fn receive_b_response(&mut self) -> Result<()> {
            let socket_open =
                self.receiver_b.receive_one(move |_: (), _| async { panic!() }).await?;
            assert!(socket_open);
        ...
        async fn receive_a_response(&mut self) -> Result<()> {
            let socket_open =
                self.receiver_a.receive_one(move |_: (), _| async { panic!() }).await?;
            assert!(socket_open);
            Ok(())
        }
    }
    ...
    async fn ask() {
        ...
        fix.sender_b
            .respond(&Response { message: format!("hello {req:?}") }, message_id.unwrap())
            .await
            .unwrap();

        fix.receive_a_response().await.unwrap();
        let res = ask_recv.try_recv().unwrap();
        assert_eq!(res, Response { message: "hello A".into() });
    }
    ...
    async fn unsolicited_response() {
        ...
        fix.sender_a
            .respond(&Response { message: "hi".into() }, RpcMessageId::INITIAL)
            .await
            .unwrap();
        assert_eq!(
    ...
    async fn socket_close() {
        ...
        let socket_open =
            fix.receiver_b.receive_one(move |_: (), _| async { panic!() }).await.unwrap();
        assert!(!socket_open);
    }
}
