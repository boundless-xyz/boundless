...
use crate::{
    ...
    contracts::{
        ...
        FulfillmentData, FulfillmentDataType, ProofRequest, RequestError,
    },
    ...
    P: Provider<Ethereum> + 'static + Clone,
{
    ...
    pub async fn wait_for_request_fulfillment(
        ...
        expires_at: u64,
    ) -> Result<(FulfillmentDataType, Bytes, Bytes), ClientError> {
        Ok(self
            .boundless_market
    ...
    pub async fn fetch_set_inclusion_receipt(
        ...
        // verifier.
        let (fulfill_type, fulfillment_data, seal) =
            self.boundless_market.get_request_fulfillment(request_id).await?;
        let fulfillment_data = FulfillmentData::decode_with_type(fulfill_type, fulfillment_data)
            .map_err(|e| ClientError::Error(anyhow!("Failed to decode fulfillment data: {}", e)))?;
