...
impl SteelCommitment {
    /// An empty [SteelCommitment] used in the [AssessorJournal] to represent the None value.
    ///
    /// Does not correspond to any block, and so its inclusion cannot be verified in any chain.
    pub const EMPTY: Self = Self { id: U256::ZERO, digest: B256::ZERO, configID: B256::ZERO };
}
...
impl RequestId {
    ...
    /// Check for non-zero bits that do not correspond to any known field/flag.
    fn unknown_bits_set(value: U256) -> bool {
        value >> 193 != U256::ZERO
    }
}
...
impl TryFrom<U256> for RequestId {
    ...
    fn try_from(value: U256) -> Result<Self, Self::Error> {
        if RequestId::unknown_bits_set(value) {
...
impl ProofRequest {
    ...
    pub fn client_address(&self) -> Result<Address, RequestError> {
        Ok(RequestId::try_from(self.id)?.addr)
    }

    /// Returns true if the authorization should be an ERC-1271 smart contract signature.
    pub fn is_smart_contract_signed(&self) -> Result<bool, RequestError> {
        Ok(RequestId::try_from(self.id)?.smart_contract_signed)
    }
...
mod tests {
    ...
    async fn test_request_id_to_parts() {
        ...
        let request_id1 = RequestId::try_from(
        ...
        assert_eq!(request_id1.index, 100);
        assert!(!request_id1.smart_contract_signed);

        // Test case 2: Smart contract signature
        let request_id2 = RequestId::try_from(
        ...
        assert_eq!(request_id2.index, 42);
        assert!(request_id2.smart_contract_signed);
    }
}
