...
const SIGNAL_REQUESTOR: Address = alloy::primitives::address!("734df7809c4ef94da037449c287166d114503198");
        ...
        match GuestEnv::decode(&input_data) {
            Ok(guest_env) => {
                ...
                match bytemuck::try_cast_slice::<u8, u32>(&guest_env.stdin) {
                    ...
                    Err(e) => {
                        tracing::debug!(
                            "Failed to convert stdin bytes to u32 words: {}",
                            e
                        );
                        None
                    }
    ...
    ANP: Provider<AnyNetwork> + 'static + Clone,
{
    pub(super) async fn process_cycle_counts(
        ...
        tracing::debug!(
            "Processing cycle counts for {} new requests",
            new_requests.len()
        );
...
mod tests {
    ...
    fn test_compute_cycle_counts_requestor_1_with_valid_input() {
        ...
        let (program_cycles, total_cycles) = cycles.unwrap();
        assert_eq!(program_cycles, 67108864, "Expected program_cycles 67108864, got {}", program_cycles);
        // Verify total_cycles = program_cycles * 1.0158
        let expected_total = program_cycles_to_total(program_cycles);
        assert_eq!(total_cycles, expected_total, "total_cycles should be program_cycles * 1.0158");
    }
    ...
    fn test_compute_cycle_counts_requestor_with_url_input() {
        ...
        assert!(
            cycles.is_none(),
            "URL inputs should not extract cycle counts"
        );
    }
    ...
    fn test_compute_cycle_counts_unknown_requestor() {
        ...
        assert!(
            cycles.is_none(),
            "Unknown requestors should return None (PENDING)"
        );
    }
    ...
    fn test_try_extract_program_cycles_invalid_hex() {
        ...
        assert!(
            program_cycles.is_none(),
            "Invalid hex data should return None"
        );
    }
    ...
    fn test_try_extract_program_cycles_url_input() {
        ...
        assert!(
            program_cycles.is_none(),
            "URL input type should return None"
        );
    }
