...
async fn test_e2e() {
    let fixture = MarketTestFixture::new().await.unwrap();
    
    let mut cli_process = IndexerCliBuilder::new(
        fixture.test_db.db_url.clone(),
        fixture.anvil.endpoint_url(),
    ...
    assert_eq!(summary.locked_orders_fulfillment_rate, 100.0, "Expected 100% fulfillment rate");
...
async fn test_aggregation_across_hours() {
    ...
    lock_and_fulfill_request(&fixture.ctx, &fixture.prover, &request1, client_sig1.as_bytes().to_vec().into()).await.unwrap();
    ...
    lock_and_fulfill_request(&fixture.ctx, &fixture.prover, &request2, client_sig2.as_bytes().to_vec().into()).await.unwrap();
...
async fn test_aggregation_percentiles() {
    ...
    for i in 0..num_requests {
        ...
        fixture.ctx.prover_market.lock_request(&request, client_sig.as_bytes(), None).await.unwrap();
...
async fn test_request_status_happy_path(_pool: sqlx::PgPool) {
    ...
    // Fulfill request
    lock_and_fulfill_request(&fixture.ctx, &fixture.prover, &req, sig.clone()).await.unwrap();
    wait_for_indexer().await;
