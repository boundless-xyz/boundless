...
mod tests {
    ...
    ) -> (AnvilInstance, Submitter<impl Provider + WalletProvider + Clone + 'static>, DbObj, usize)
    {
        ...
        let market = BoundlessMarketService::new_for_broker(market_address, provider.clone(), prover_addr);
        market.deposit_collateral_with_permit(default_allowance(), &signer).await.unwrap();

        let market_customer =
            BoundlessMarketService::new_for_broker(market_address, customer_provider.clone(), customer_addr);
