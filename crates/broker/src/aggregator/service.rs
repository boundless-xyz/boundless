...
impl AggregatorService {
            ...
            let fulfillment_data =
                match order.request.requirements.predicate.predicateType {
                    ...
                    _ => FulfillmentData::from_image_id_and_journal(
                        <[u8; 32]>::from(Digest::from_hex(order.image_id.with_context(|| {
                            format!("Missing image_id for order: {order_id}")
                        })?)?),
                        journal,
                    ),
                };
    ...
    async fn get_combined_journal_size(&self, order_ids: &[String]) -> Result<usize> {
        ...
        for order_id in order_ids {
            ...
            // The journal is cached by the prover that produced this proof_id.
            let selector = order.request.requirements.selector;
            let order_prover = self
                .backends
                .find(selector)
...
mod tests {
    ...
    async fn aggregate_order_incremental() {
        ...
        let agg_state = aggregating_batch.aggregation_state.expect("aggregation state present");
        let view = boundless_r0_backend::decode_set_builder_state(&agg_state.state).unwrap();
        assert!(!view.guest_state.mmr.is_finalized());
