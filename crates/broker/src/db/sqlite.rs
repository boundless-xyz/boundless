...
//! SQLite-backed [`BrokerDb`] implementation.
//!
//! Owns the connection pool, runs migrations on startup, and implements the
//! per-method SQL. Also hosts the `broker_sqlite_url_for_chain` helper that
//! derives a per-chain URL from the broker's base configuration (one DB file
//! per chain ID, named in-memory base mapped distinctly per chain).
