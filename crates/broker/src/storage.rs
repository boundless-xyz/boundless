...
pub async fn upload_image_uri(
    ...
    if prover.has_image(&image_id_str).await? {
        tracing::debug!("Skipping program upload for cached image ID: {image_id_str} for request {:x}", request.id);
        return Ok(image_id_str);
    }

    tracing::debug!("Fetching program for request {:x} with image ID {image_id_str} from URI {}", request.id, request.imageUrl);
    ...
    let image_id =
        risc0_zkvm::compute_image_id(&image_data).context(format!("Failed to compute image ID for request {:x}", request.id))?;
    ...
    tracing::debug!("Uploading program for request {:x} with image ID {image_id_str} to prover", request.id);
    prover
