...
```bash
cargo install --locked --git https://github.com/boundless-xyz/boundless boundless-cli --branch release-0.15 --bin boundless
```
...
For all relevant commands below: `stake`, `delegate-rewards` and ` submit`, the CLI will require a private key denoted by the environment variable `PRIVATE_KEY` i.e. the private key for the `POVW_LOG_ID`. 
...
```bash
boundless zkc balance-of ${POVW_LOG_ID:?}
```
...
```bash
boundless povw claim --beacon-api-url${BEACON_CHAIN_RPC_ENDPOINT} --rpc-url ${RPC_URL} --povw-accounting-address ${POVW_ACCOUNTING_ADDRESS} --povw-mint-address ${POVW_MINT_ADDRESS}
```
