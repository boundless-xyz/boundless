...
For the purposes of this section, "miner" will be used to refer to somebody who stakes *$ZKC* and submits valid work regularly. This term could be used interchangebly with prover, which is used commonly throughout the Boundless docs.
...
For mining rewards, delegation is a recommended practice as it allows for separation between the wallet that stakes *$ZKC* and the wallet specified in Bento in work proofs as the `REWARDS_ADDRESS`. For more information, please see [PoVW Quick Start](/zkc/povw/quick-start).
...
For mining rewards, it is recommended to set `<DELEGATEE>` as the address set to `REWARDS_ADDRESS` in Bento. The private key should denote the address that you want to delegate *from* i.e. the wallet with an active stake position. 
