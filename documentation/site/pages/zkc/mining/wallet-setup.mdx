...
To [enable ZK mining](/zkc/mining/enable), the prover specifies the reward address which must be a valid Ethereum address. This address *does not* have to be the same as the *$ZKC* staking wallet address or a Boundless prover address, though it can be used for all three purposes. 
