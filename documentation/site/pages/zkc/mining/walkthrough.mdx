...
```bash
boundless rewards prepare-mining --new ${REWARD_ADDRESS} --state-file ${STATE_FILE_LOCATION} --from-bento
```
...
```bash
boundless rewards prepare-mining --state-file ${STATE_FILE_LOCATION} --from-bento
```
...
```bash
boundless rewards prepare-mining --from-bento
```
