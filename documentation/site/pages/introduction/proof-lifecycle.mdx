...
# async fn submit_request(request: ProofRequest, signer: impl Signer) -> Result<(), Box<dyn std::error::Error>> {
# let boundless_client = ClientBuilder::default().build().await?;
# let (request_id, expires_at) = boundless_client.submit_request(&request, &signer).await?;
