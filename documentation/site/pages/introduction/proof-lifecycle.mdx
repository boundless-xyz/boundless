...
They will then run the zkVM software to generate a proof, at which point this proof will be sent back on-chain for the requestor to access.
...
- So, what does a proof do for your application?
  ...
  - This allows you to offload execution from your application (on-chain to off-chain).
  - Allow for the same trust assumptions as on-chain (magic of ZK).
  - Provide a portable way to verify your program both off-chain and on-chain.
- That's great, show me the good stuff:
  - Workflow diagram of a standard on-chain application (think excalidraw) utilising a ZK proof to offload execution to the prover.
