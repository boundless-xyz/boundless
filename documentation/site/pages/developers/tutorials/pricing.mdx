...
The following two sections detail some recommended guidelines when setting the request parameters. These guidelines are formulas, and they are to be taken with a pinch of salt. Please remember to start conservatively when tuning each parameter. Provers have to respond and some may set their pricing strategy based on each requestor's standard request profile. Therefore, it is recommended to change request parameters slowly during testing, and *only* once proof fulfillment becomes stable, to push closer to the desired end goal. 
