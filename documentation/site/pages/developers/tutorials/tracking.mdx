...
match client.wait_for_request_fulfillment(request_id, check_interval, expires_at).await {
    Ok((fulfillment_data_type, fulfillment_data, seal)) => {
        let fulfillment_data = FulfillmentData::decode_with_type(fulfillment_data_type, fulfillment_data)?;

        // Process the fulfilled proof
        tracing::info!("Proof received with fulfillment data: {:?}", fulfillment_data);
    }
...
match status {
    RequestStatus::Fulfilled => {
        // Retrieve the fulfillment data and seal for the fulfilled request
        let (fulfillment_data_type, fulfillment_data, seal) = client
