...
Each request specifies a set of request parameters (see [request.yaml](https://github.com/boundless-xyz/boundless/blob/main/request.yaml)); these parameters specify the request ID, the proof requirements, the URL for the relevant guest program and its inputs, and *the auction parameters*.  

This guide helps requestors understand, configure and optimize their requests via these *offer parameters*. Requestors, who understand these offer parameters, will be able to more effectively tailor their request auctions to current market conditions. This is essential for getting timely and cost-effective proofs. 
...
### Where are these offer parameters specified?

Requestors may either use a request.yaml file 
...
Let's walk through an example request and figure out how to set the offer parameters based on the recommendations 



500MCycle proof. We can calculate the estimated execution and proving times using [this calculator](/developers/tutorials/configure#estimate-the-executionproving-time-of-a-request).



 with the following offer parameters specified in the [request.yaml](https://github.com/boundless-xyz/boundless/blob/main/request.yaml):
...
offer:
    ...
    rampUpStart: 60 # seconds
    rampUpPeriod: 50 # blocks
    lockTimeout: 1200 # 20 minutes
    timeout:  # 1 hour 
```
...
- For 60 seconds (specified by `rampUpStart`), the order will be at the minimum price (`minPrice`). This time allows for provers to see the request, and run checks on the request (known as a preflight)[^1]. During this 60 second period, provers can lock the order at the minimum price. 
- After 60 seconds, 


- After order creation and before the ramp up start time i.e. 60 seconds, the order is surfaced to the market. In theory, setting this value into the future should minimize the advantage for provers who run execution vs provers who skip it. It also prevents provers from locking proofs in the same block as the request.
- At 60 seconds, the price is set to the minimum price of 0.001 $ETH and the bidding officially starts.
...
## Setting Optimal Offer Parameters
