...
|----------------|-------------------|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Ramp Up Start  | `rampUpStart`     | seconds  | An alloted period of time before the auction price begins to increase, allowing provers to analyze jobs before the price increases. Provers can still lock during this period. |
...
- After the "lock timeout", a race amongst secondary provers begins. They have until the "timeout" (1 hour from request creation, 15 minutes from the primary prover deadline) to fulfill the request; otherwise the order expires.
...
|----------------|---------|--------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| Ramp Up Start  | seconds  | `(cycle_count / 30 000 000) + 10` | This time period allows provers to preflight the request; the duration of the preflight depends on the size of the program. This recommendation assumes an executor speed of 30 MHz, but in practice this can be tuned up or down depending on the maturity of provers and their setups on the protocol.  |
| Ramp Up Period | blocks  | 100 | On Base mainnet, 100 blocks is ~200 seconds or 3min20s. If you require lower latency, experiment with dropping this figure. |
| Lock Timeout   | seconds | 600 |  |
| Timeout        | seconds | 1200|TODO|
