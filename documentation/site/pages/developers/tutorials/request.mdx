...
# program: &'static [u8],
# input: &[u8]) -> Result<()> {
// Create a request using request_params
let request_params = client.request_params()
...
let (request_id, expires_at) = client.submit_onchain(request_params).await?;
# Ok(())
# }
```
</StripRustCodeComments>
...
Using `client.submit_offchain(request_params)`, you can send your request offchain through the order-stream service.
