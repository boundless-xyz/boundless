...
let (fulfillment_data_type, fulfillment_data, seal) = client.boundless_market.get_request_fulfillment(request_id).await?;
...
async fn proof_submission(signer: &PrivateKeySigner, rpc_url: Url) -> anyhow::Result<()> {
    ...
    println!("FulfillmentData: {fulfillment_data:?}, Seal: {seal:?}");
