...
- `GuestEnvBuilder`: Prepare inputs for the guest by construsting `GuestEnv`.
...
use boundless_market::{
  client::Client,
  contracts::{ProofRequest, Input, RequestId, Requirements, Predicate, Offer},
...
async fn proof_submission(signer: &PrivateKeySigner, rpc_url: Url, market_address: Address, set_verifier_address: Address) -> anyhow::Result<()> {
    ...
    let program_url = client.upload_program(&std::fs::read("guest.bin")?).await?;
    let input_url = client.upload_input(&Input::builder().write(&42u32)?.build_vec()?).await?;
