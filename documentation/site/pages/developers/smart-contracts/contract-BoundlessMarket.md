...
*The fee is configured as a constant to avoid accessing storage and thus paying for the
...
*When called within the same transaction, this method can be used to fulfill a request
...
*It is possible for anyone to fulfill a request at any time while the request has not expired.
...
*If a never locked request is fulfilled, but fails the requirements for payment, no
...
*This function is called after payment is processed and handles any callback specified in the request*
...
*The provers stake has already been transferred to the contract when the request was locked.
...
*Note it is possible for a request to result in a slash, but still be fulfilled
