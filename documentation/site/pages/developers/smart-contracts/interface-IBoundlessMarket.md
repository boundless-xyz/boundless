...
*Submitting the transaction only broadcasts it, and is not a required step.
...
*Note that the signature is not verified by the contract and should instead be verified
...
*It is possible for this event to be logged multiple times for a single request. This
...
*This can happen if a client signs multiple requests with the same ID (i.e. multiple
