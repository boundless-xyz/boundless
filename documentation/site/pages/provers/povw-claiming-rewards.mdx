...
Proof of Verifiable Work (PoVW), a novel invention within Boundless, is a permissionless incentive mechanism which enables provers to be rewarded in $ZKC in exchange for their proving work on the Boundless marketplace or elsewhere. Provers generating unique zero-knowledge proofs are rewarded with $ZKC. 

## Prerequisites 

### Installing the latest Boundless CLI 

PoVW CLI functionality was added in v0.XX and above. To install the latest release, run the following command:

```bash
TODO
```

### Enabling PoVW in Bento

PoVW was added to RISC Zero’s zkVM ([R0VM](https://github.com/risc0/risc0)) in version 3.0.1 (the implementation PR can be found [here](https://github.com/risc0/risc0/pull/3220)). It has been added to Bento; to enable PoVW requires Bento version TBD and above ([implementation PR](https://github.com/boundless-xyz/boundless/pull/1044)). 

To enable PoVW in Bento, first make sure to stop Bento safely. If Bento is backing the Broker, please carry out *Step 1* from [Safe Upgrade Steps](https://docs.boundless.network/provers/broker#safe-upgrade-steps) and then return to this page. 
...
:::tip[Tip]
Provers can use their Log ID as their $ZKC staking address; for simplicity, the following tutorial uses the Log ID to stake $ZKC as well. In practice, provers may decide to use a separate wallet to stake $ZKC, and this is [explained](#TODO) later. 
:::
...
:::tip[Tip]
It is helpful to read both the **Terminology** and **Reward Claiming Workflow** sections before running the commands listed below.
:::
...
:::tip[Tip]
This section assumes a local `cast` installation which requires [Foundry](https://getfoundry.sh/) to be installed.
:::
...
### Reward Epoch Section (not sure if this belongs here)
...
```bash
boundless povw prove-update --new ${POVW_LOG_ID} --state <DESIRED_STATE_FILE_LOCATION>
```
