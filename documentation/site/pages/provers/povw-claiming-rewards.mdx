...
Provers use the Boundless CLI to stake $ZKC, prepare work log updates, and submit them onchain. To install the Boundless CLI, run:

```bash
cargo install --locked --git https://github.com/boundless-xyz/boundless boundless-cli --branch release-0.14 --bin boundless
```
...
Provers will need the private key for this address when using the PoVW CLI to prepare and submit valid work to the PoVW contracts.  Please see [Provers submit a Work Log Update](/provers/povw-claiming-rewards#provers-submit-a-work-log-update-with-send-update)  for more information.
...
:::warning[Provers will require funds on Ethereum mainnet]
For all relevant commands below: `stake`, `delegate-rewards` and ` send-update`, the CLI will require a private key denoted by the environment variable `PRIVATE_KEY` i.e. the private key for the `POVW_LOG_ID`. 

This key is used for sending Ethereum transactions is stored in the environment variable `PRIVATE_KEY` 

Therefore, POVW_LOG_ID` will require funds on Ethereum mainnet to pay for gas. 
:::
...
Before provers can begin contributing work to claim PoVW rewards, they *must stake* $ZKC. 
...
```bash
boundless povw prove-update --state ${STATE_FILE_LOCATION} --from-bento
```
...
```bash
boundless povw send-update --state ${STATE_FILE_LOCATION} --rpc-url ${RPC_URL}
```
...
```bash
boundless povw claim-reward --beacon-api-url${BEACON_CHAIN_RPC_ENDPOINT} --rpc-url ${RPC_URL} --povw-accounting-address ${POVW_ACCOUNTING_ADDRESS} --povw-mint-address ${POVW_MINT_ADDRESS}
```
...
```bash
cast call ${ZKC_ADDRESS:?} 'getEpochEndTime(uint256) returns (uint256)' <EPOCH_NUMBER>
```
...
| Term | Description |
|------|-------------|
| **Log ID** | This is the Ethereum address set with the `POVW_LOG_ID` environment variable. This address can be the same as the one used for a prover on the Boundless Market, but it doesn't have to be. See [Recommended Practices](/provers/povw-claiming-rewards#recommended-practices) for more information. |
| **Work Receipts** | For each proving workload, PoVW generates work receipts alongside the expected ZK proof. These work receipts are compressed into a “work log” update to post onchain. This work log update represents the combined work of all compressed work receipts. Work receipts are compressed using the `prove-update` CLI command, and this process is entirely local. |
...
#### Provers compress work receipts with `prove-update`

The `prove-update` command compresses all local work receipts to reflect work done during any given epoch - this update is stored in a local “state” file.  

#### Provers submit a work log update with `send-update`

Towards the end of an epoch, the prover must verify their work done to the PoVW contracts using the `send-update` command. This will sign the work log update using the Log ID’s private key and post the update to the PoVW contracts.   
...
To check details of the current epoch, please see [Checking Details of Current Reward Epoch](/provers/povw-claiming-rewards#checking-details-of-current-reward-epoch),

#### Provers claim rewards with `claim-rewards`

Once the epoch has been finalized, the `claim-reward` command can be used to claim any unclaimed rewards; `claim-reward` uses Steel to query all work log updates and verify reward calculations. 

`claim-reward` requires *both* a valid archive node RPC endpoint and a Beacon Chain RPC endpoint ([Quicknode](https://www.quicknode.com/) offers both types of nodes with their paid plan).
 
Claiming rewards does not have to be done every epoch; all unclaimed rewards from past epochs will be claimed automatically on the next valid `claim-reward` call.
...
When [enabling PoVW in Bento](/provers/povw-claiming-rewards#specify-the-log-id-address), provers specify the following environment variables:
...
Provers can now follow the [staking instructions](/provers/povw-claiming-rewards#staking-zkc) to stake $ZKC from the `ZKC_WALLET_ADDRESS`. Once staked, provers must delegate rewards i.e. associate the `POVW_LOG_ID` with `ZKC_WALLET_ADDRESS`. This is done with the following command:
...
Once the rewards are delegated, the prover can continue with the normal reward claiming workflow and they can [start proving](/provers/povw-claiming-rewards#start-proving).
