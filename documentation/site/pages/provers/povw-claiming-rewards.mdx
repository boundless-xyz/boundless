...
Provers can omit `--new ${POVW_LOG_ID}` once the state file is created. For further updates to the work log, run the following:
...
```bash
boundless povw claim-rewards --beacon-chain-rpc ${BEACON_CHAIN_RPC_ENDPOINT} --archive-node-rpc ${ARCHIVE_NODE_RPC_ENDPOINT}
```
