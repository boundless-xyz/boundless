...
| events\_poll\_blocks | Number of blocks to query per batch when polling for new market events. After the initial lookback completes, the market monitor will query a maximum of this many blocks at a time when polling for new events. Default: 150. |
...
| lookback\_blocks | On startup, the number of previous blocks to search for possible open orders. |
