...
The below components are built on top of the core infrastructure. These components are the basis for Bento and are critical for its operation:
...
The REST API provides a external interface to start / stop / monitor jobs and tasks within TaskDB. For now, the [Bonsai API docs](https://api.bonsai.xyz/swagger-ui/) provide a good reference for the Bento REST API.
