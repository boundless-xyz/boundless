...
struct Args {
    ...
    /// Address of the requestor contract.
    #[clap(short, long, env)]
    requestor_smart_contract_address: Address,
}
...
async fn main() -> Result<()> {
    ...
    run(
        ...
        args.requestor_smart_contract_address,
    )
    .await?;
...
async fn run(
    ...
    let request_id = RequestId::new(requestor_smart_contract_address, days_since_epoch)
...
mod tests {
    ...
    async fn test_main() {
        ...
        let (smart_contract_requestor_address, smart_contract_owner) =
            deploy_smart_contract_requestor(&anvil, &ctx).await.unwrap();

        let value_to_fund = parse_ether("0.5").unwrap();

        // let fund_smart_contract_tx = ctx
        //     .customer_provider
        //     .send_transaction(
        //         TransactionRequest::default()
        //             .to(smart_contract_requestor_address)
        //             .value(value_to_fund),
        //     )
        //     .await
        //     .unwrap();
        // fund_smart_contract_tx.watch().await.unwrap();

        let smart_contract_requestor = SmartContractRequestorInstance::new(
            smart_contract_requestor_address,
            smart_contract_owner.clone(),
        )
        .clone();

        let balance_before = smart_contract_owner
            .get_balance(smart_contract_owner.default_signer_address())
            .await
            .unwrap();
        tracing::info!(
            "Address of request contract: {} Balance before: {} Owner: {}",
            smart_contract_requestor_address,
            balance_before,
            smart_contract_owner.default_signer_address()
        );

        let deposit_call = IBoundlessMarket::depositCall {}.abi_encode();

        let requestor_provider = smart_contract_requestor.provider();
        let requestor_provider_address = requestor_provider.default_signer_address();
        let requestor_provider_bal = requestor_provider
            .get_balance(requestor_provider.default_signer_address())
            .await
            .unwrap();

        tracing::info!(
            "Requestor provider address: {} Balance after: {}",
            requestor_provider_address,
            requestor_provider_bal
        );

        let pending_tx = smart_contract_requestor
