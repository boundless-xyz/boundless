...
import {
    ...
    FulfillmentDataType
} from "../../src/legacy/types/FulfillmentData.sol";
import {RequestId} from "../../src/legacy/types/RequestId.sol";
import {AssessorCallback} from "../../src/legacy/types/AssessorCallback.sol";
import {BoundlessMarketLib} from "../../src/legacy/libraries/BoundlessMarketLib.sol";
import {MerkleProofish} from "../../src/legacy/libraries/MerkleProofish.sol";
import {ProofRequest} from "../../src/legacy/types/ProofRequest.sol";
import {LockRequest} from "../../src/legacy/types/LockRequest.sol";
...
import {Predicate, PredicateLibrary, PredicateType} from "../../src/legacy/types/Predicate.sol";
...
import {Selector} from "../../src/legacy/types/Selector.sol";
...
contract BoundlessMarketLegacyViaFallbackTest is Test {
...
contract BoundlessMarketLegacyViaFallbackBasicTest is BoundlessMarketLegacyViaFallbackTest {
    using ReceiptClaimLib for ReceiptClaim;
    using BoundlessMarketLib for Offer;
    using BoundlessMarketLib for ProofRequest;
    using SafeCast for uint256;
    ...
    function testLockRequestWithSignatureProverSignatureIncorrectRequest() public {
        ...
        // The recovered address differs from the standalone legacy suite by one
        // CREATE nonce: setUp here also deploys the new market impl before the
        // proxy, so the proxy address (and thus the EIP-712 domain separator)
        // shifts. The expected address is the deterministic recovery against
        // this configuration.
        vm.expectRevert(
            abi.encodeWithSelector(
                IBoundlessMarket.InsufficientBalance.selector, address(0xf9D65aDD060EeC50A7e86C29d91fBEAaC0eDe727)
            )
        );
    ...
    function testLockRequestWithSignatureProverSignatureIncorrectDomain() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IBoundlessMarket.InsufficientBalance.selector, address(0x27940eD27511Eef63A19320520D3fC30a4F35a56)
            )
        );
        boundlessMarket.lockRequestWithSignature(request, clientSignature, badProverSignature);
...
contract BoundlessMarketLegacyViaFallbackBench is BoundlessMarketLegacyViaFallbackTest {
    using BoundlessMarketLib for Offer;
...
contract BoundlessMarketLegacyViaFallbackUpgradeTest is BoundlessMarketLegacyViaFallbackTest {
    using BoundlessMarketLib for Offer;
