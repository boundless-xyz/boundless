...
/// Shanghai-compatible variant: uses sstore/sload instead of tstore/tload.
...
library FulfillmentContextLibrary {
    ...
    /// @notice Loads and clears the stored context in one operation.
    /// Clearing prevents permanent storage growth (replaces tstore auto-clear semantics).
    /// @param requestDigest The storage key to load from and clear
    /// @return The loaded and unpacked FulfillmentContext struct
    function load(bytes32 requestDigest) internal returns (FulfillmentContext memory) {
        uint256 packed;
        assembly {
