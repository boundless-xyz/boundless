...
struct Deployment {
    ...
    /// Address of the RISC Zero stack verifier router (upstream verifier infrastructure).
    address risc0Router;
    /// Address of the RISC Zero stack timelock controller (upstream verifier infrastructure).
    address risc0TimelockController;
    /// Min delay configured on the RISC Zero stack timelock controller.
    uint256 risc0TimelockDelay;
    /// Deployed RISC Zero stack verifier implementations.
    VerifierDeployment[] risc0Verifiers;
}
...
library DeploymentLib {
    ...
    function copyTo(Deployment memory mem, Deployment storage stor) internal {
        ...
        stor.risc0Router = mem.risc0Router;
        stor.risc0TimelockController = mem.risc0TimelockController;
        stor.risc0TimelockDelay = mem.risc0TimelockDelay;
        delete stor.risc0Verifiers;
        for (uint256 i = 0; i < mem.risc0Verifiers.length; i++) {
            stor.risc0Verifiers.push(mem.risc0Verifiers[i]);
        }
    }
}
...
library ConfigParser {
    ...
    function parseConfig(string memory config, string memory chainKey) internal view returns (Deployment memory) {
        ...
        // Parse RISC Zero stack fields (upstream verifier infrastructure).
        deploymentConfig.risc0Router = stdToml.readAddressOr(config, string.concat(chain, ".risc0-router"), address(0));
        deploymentConfig.risc0TimelockController =
            stdToml.readAddressOr(config, string.concat(chain, ".risc0-timelock-controller"), address(0));
        deploymentConfig.risc0TimelockDelay =
            stdToml.readUintOr(config, string.concat(chain, ".risc0-timelock-delay"), uint256(0));

        // Iterate over the risc0-verifiers struct entries to get the length.
        uint256 risc0VerifiersLength = 0;
        string memory risc0VerifierKey =
            string.concat(chain, ".risc0-verifiers[", VM.toString(risc0VerifiersLength), "]");
        while (stdToml.keyExists(config, risc0VerifierKey)) {
            risc0VerifiersLength++;
            risc0VerifierKey = string.concat(chain, ".risc0-verifiers[", VM.toString(risc0VerifiersLength), "]");
        }
        deploymentConfig.risc0Verifiers = new VerifierDeployment[](risc0VerifiersLength);

        // Iterate over the risc0-verifiers struct entries and parse them.
        uint256 risc0VerifierIndex = 0;
        risc0VerifierKey = string.concat(chain, ".risc0-verifiers[", VM.toString(risc0VerifierIndex), "]");
        while (stdToml.keyExists(config, risc0VerifierKey)) {
            VerifierDeployment memory risc0Verifier;
            risc0Verifier.name = stdToml.readStringOr(config, string.concat(risc0VerifierKey, ".name"), "");
            risc0Verifier.version = stdToml.readStringOr(config, string.concat(risc0VerifierKey, ".version"), "");
            risc0Verifier.selector =
                bytes4(stdToml.readUint(config, string.concat(risc0VerifierKey, ".selector")).toUint32());
            risc0Verifier.verifier = stdToml.readAddress(config, string.concat(risc0VerifierKey, ".verifier"));
            risc0Verifier.estop = stdToml.readAddress(config, string.concat(risc0VerifierKey, ".estop"));
            risc0Verifier.unroutable = stdToml.readBoolOr(config, string.concat(risc0VerifierKey, ".unroutable"), false);
            risc0Verifier.stopped = stdToml.readBoolOr(config, string.concat(risc0VerifierKey, ".stopped"), false);

            deploymentConfig.risc0Verifiers[risc0VerifierIndex] = risc0Verifier;
