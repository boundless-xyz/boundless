...
/// @notice A struct for storing validated fulfillment information in persistent storage.
/// @dev This struct is designed to be packed into a single uint256 for efficient storage.
/// Shanghai-compatible variant: uses sstore/sload instead of tstore/tload. Selected over the
/// transient-storage variant via the `fulfillment-context/` remapping in the `shanghai` profile.
...
library FulfillmentContextLibrary {
    ...
    /// @notice Packs and stores the object to persistent storage
    /// @param x The FulfillmentContext struct to store
    /// @param requestDigest The storage key (used directly as the sstore slot)
    function store(FulfillmentContext memory x, bytes32 requestDigest) internal {
        uint256 packed = pack(x);
        assembly {
            sstore(requestDigest, packed)
        }
    }

    /// @notice Loads from persistent storage and unpacks to FulfillmentContext.
    /// @dev Non-destructive (mirrors the transient `tload`), so the same digest may be read more
    ///      than once within a fulfillment. The slot is released separately via `clear`.
    ...
    function load(bytes32 requestDigest) internal view returns (FulfillmentContext memory) {
        uint256 packed;
        assembly {
            packed := sload(requestDigest)
        }
        return unpack(packed);
    }

    /// @notice Releases the stored context once it has been consumed.
    /// @dev Clears the slot to bound permanent storage growth and to enforce the same
    ///      single-transaction price-then-fulfill semantics that transient storage provides
    ///      automatically (a context cannot survive into a later transaction).
    /// @param requestDigest The storage key to release
    function clear(bytes32 requestDigest) internal {
        assembly {
            sstore(requestDigest, 0)
        }
    }
}
