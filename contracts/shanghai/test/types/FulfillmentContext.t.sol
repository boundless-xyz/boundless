...
contract FulfillmentContextLibraryTest is Test {
    ...
    /// forge-config: default.fuzz.runs = 10000
    function testFuzz_StoreAndLoadAndClear(bool valid, bool expired, uint96 price) public {
        FulfillmentContext memory original = FulfillmentContext({valid: valid, expired: expired, price: price});
        ...
        // Load and clear the FulfillmentContext from the specified slot
        ...
        FulfillmentContext memory cleared = FulfillmentContextLibrary.load(slot);
        assertEq(cleared.valid, false, "Should be cleared: valid");
        assertEq(cleared.expired, false, "Should be cleared: expired");
        assertEq(cleared.price, 0, "Should be cleared: price");
    }
}
