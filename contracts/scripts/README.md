...
3. Dry run the upgrade of the proof market and proxy:
...
4. Send the upgrade transactions for the proof market by running the command again with `--broadcast`.

   > [!NOTE]
   > When using Fireblocks, sending a transaction to a particular address may require allow-listing it.

5. Load the upgraded ProofMarket (proxy) address into the environment:
