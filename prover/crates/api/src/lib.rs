...
fn is_gpu_worker_stream(task_stream: &str) -> bool {
    matches!(task_stream, PROVE_WORK_TYPE | JOIN_WORK_TYPE | COPROC_WORK_TYPE | SNARK_WORK_TYPE)
}
...
pub struct Args {
    ...
    /// Age in hours after which uploaded inputs are deleted from local storage.
    /// Set to 0 to disable cleanup.
    #[clap(env, long, default_value_t = 24)]
    input_ttl_hours: u64,
}
...
pub async fn run(args: &Args) -> Result<()> {
    ...
    if args.input_ttl_hours > 0 {
        ...
        tokio::spawn(async move {
            loop {
                purge_old_inputs(&inputs_dir, ttl).await;
...
async fn purge_old_inputs(inputs_dir: &std::path::Path, ttl_hours: u64) {
    let cutoff = std::time::SystemTime::now() - std::time::Duration::from_secs(ttl_hours * 60 * 60);

    let dir = match std::fs::read_dir(inputs_dir) {
        Ok(d) => d,
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return,
        Err(e) => {
            tracing::warn!("Failed to read inputs dir for cleanup: {e}");
            return;
        }
    };
    ...
    for entry in dir.flatten() {
        ...
        if modified < cutoff {
            if let Err(e) = std::fs::remove_file(&path) {
                tracing::warn!("Failed to delete old input {}: {e}", path.display());
            } else {
                removed += 1;
            }
        }
    ...
    if removed > 0 {
        tracing::info!("Input TTL cleanup: removed {removed} file(s) older than {ttl_hours}h");
    }
}
