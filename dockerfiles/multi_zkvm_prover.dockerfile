...
RUN --mount=type=secret,id=ci_cache_creds,target=/root/.aws/credentials \
    --mount=type=cache,target=/root/.cache/sccache/,id=multi_zkvm_prover_sc \
    --mount=type=cache,target=/usr/local/cargo/registry,id=cargo_registry \
    --mount=type=cache,target=/src/target,id=multi_zkvm_prover_target \
    source dockerfiles/sccache-config.sh ${S3_CACHE_PREFIX} && \
    RELEASE_FLAG="" && \
    if [ "${RELEASE}" = "true" ]; then RELEASE_FLAG="--release"; fi && \
    cargo chef cook ${RELEASE_FLAG} --recipe-path recipe.json --package multi-zkvm-prover && \
    sccache --show-stats
...
RUN --mount=type=secret,id=ci_cache_creds,target=/root/.aws/credentials \
    ...
    cargo build ${RELEASE_FLAG} --bin multi_zkvm_prover && \
    cp /src/target/${PROFILE_DIR}/multi_zkvm_prover /src/multi_zkvm_prover && \
    sccache --show-stats
...
COPY --from=builder /src/multi_zkvm_prover /app/multi_zkvm_prover

ENTRYPOINT ["/app/multi_zkvm_prover"]
